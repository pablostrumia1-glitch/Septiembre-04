...
async def stems_separate(
    ...
    if mode not in ("demucs_4stem", "vocals_hq"):
        raise HTTPException(400, f"mode inválido: '{mode}'. Validos: demucs_4stem, vocals_hq")
...
def stems_download(job_id: str, stem_name: str):
    ...
    if not stem_path or not os.path.exists(stem_path):
        raise HTTPException(410, "Stem no encontrado o expirado. Volve a separar el track.")
