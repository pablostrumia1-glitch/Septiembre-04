...
def _downsample_gr_curve(gr_db_arr: np.ndarray, sr: int, target_hz: float = 30.0) -> tuple[list, float]:
    """Reduce un array de GR por-muestra a ~target_hz puntos, para mandar al
    frontend una curva liviana que se pueda sincronizar con audio.currentTime
    durante la reproducción (medidor "en vivo" client-side, sin streaming).
    Un valor por sample de un buffer de 25s sería enorme e inútil para una
    barra visual — se usa el MÍNIMO de cada ventana (mayor reducción), que
    es más representativo para un medidor que el promedio (no esconde
    picos cortos de gain reduction dentro de la ventana)."""
    n = len(gr_db_arr)
    if n == 0:
        return [], round(1000.0 / target_hz, 3)
    hop = max(1, int(round(sr / target_hz)))
    n_points = int(np.ceil(n / hop))
    values = [round(float(np.min(gr_db_arr[i * hop:(i + 1) * hop])), 2) for i in range(n_points)]
    return values, round(1000.0 * hop / sr, 3)
...
def multiband_compressor(audio: np.ndarray, sr: int,
    ...
    if bypass:
        return audio, {"low_gr_db": 0.0, "mid_gr_db": 0.0, "high_gr_db": 0.0,
                       ...
                       "low_curve": [], "mid_curve": [], "high_curve": [], "curve_hop_ms": 0.0}
    ...
    # Curvas para el medidor "en vivo" sincronizado con audio.currentTime
    # (ver _downsample_gr_curve) — las 3 bandas comparten sr, así que el
    # hop_ms sale igual para las 3; se manda una sola vez.
...
def apply_mastering_chain(
    ...
    **_ignored,
    ...
    if comp_bypass:
        comp_meters = {"bypass": True, "gr_db": 0.0, "in_db": round(float(20*np.log10(np.sqrt(np.mean(audio**2))+1e-9)), 2), "out_db": round(float(20*np.log10(np.sqrt(np.mean(audio**2))+1e-9)), 2), "curve": [], "curve_hop_ms": 0.0}
    else:
        audio, comp_meters, _comp_gr_arr = compressor(
            audio, sr,
            ...
            return_gain_curve=True,
        )
        _comp_curve, _comp_curve_hop_ms = _downsample_gr_curve(_comp_gr_arr, sr)
        comp_meters = {**comp_meters, "bypass": False, "curve": _comp_curve, "curve_hop_ms": _comp_curve_hop_ms}
    ...
    glue_meters = {"bypass": True, "gr_db": 0.0, "curve": [], "curve_hop_ms": 0.0}
    if not glue_bypass:
        audio, glue_meters, _glue_gr_arr = compressor(
            audio, sr,
        ...
        _glue_curve, _glue_curve_hop_ms = _downsample_gr_curve(_glue_gr_arr, sr)
        glue_meters["curve"] = _glue_curve
        glue_meters["curve_hop_ms"] = _glue_curve_hop_ms
